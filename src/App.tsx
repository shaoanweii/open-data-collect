  ...
  return (
    <div className="app-shell">
      ...
      <main className="main">
          ...
          <section className="workspace single-column">
            ...
            <div className="panel glass">
              ...
              <FilterGroup<NoteType>
                ...
                items={["不限", "图文", "视频"]}
                tone="blue"
                onChange={(value) => updateFilter("note_type", value)}
              />
              <FilterGroup<PublishTime>
                ...
                tone="gold"
                onChange={(value) => updateFilter("publish_time", value)}
              />
              <FilterGroup<SearchScope>
                ...
                tone="rose"
                onChange={(value) => updateFilter("search_scope", value)}
              />
              <FilterGroup<SortBy>
                title="排序方式"
              ...
              <div className="option-grid">
                <label className="toggle-row">
                  <input
                    ...
                    onChange={(event) => updateCommentConfig({ click_more_replies: event.target.checked })}
                  />
                  <span>展开二级评论</span>
                </label>
                <label className="toggle-row">
                  <input
                    ...
                    onChange={(event) => updateOption({ includeUserProfiles: event.target.checked })}
                  />
                  <span>补充用户主页</span>
                </label>
              </div>
              ...
              <div className="number-grid">
                <NumberField
                  ...
                  min={1}
                  max={500}
                  onChange={(value) => updateOption({ searchLimit: value })}
                  optional
                />
                <NumberField
                  label="一级评论上限"
                  ...
                  min={10}
                  max={100}
                  onChange={(value) => updateCommentConfig({ max_comment_items: value ?? 10 })}
                />
                <NumberField label="并发数" value={1} min={1} max={1} onChange={() => updateOption({ maxDetailConcurrency: 1 })} />
                <NumberField
                  label="请求间隔 ms"
                  ...
                  max={5000}
                  step={500}
                  onChange={(value) => updateOption({ requestDelayMs: value ?? 1500 })}
                />
              </div>
            </div>

            {/* ---- 创建按钮 ---- */}
            <button className="primary-action" type="button" onClick={handleCreateTask} disabled={isRunning}>
        ...
        {view === "comments" && <CommentTable comments={comments} users={users} />}
        {view === "users" && <UserInsight users={users} posts={posts} comments={comments} tasks={tasks} />}
        {view === "userpool" && <UserPool users={users} posts={posts} comments={comments} />}
        {view === "clue_pool" && <CluePool />}
      </main>
    </div>
  );
}
...
function CommentTable({ comments, users }: { comments: StoredComment[]; users: StoredUser[] }) {
  const [page, setPage] = useState(1);
  const pageSize = 20;
          ...
          {stats.channelPosts.length > 0 ? (
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={stats.channelPosts} margin={{ top: 4, right: 16, left: -8, bottom: 0 }}>
                ...
                <Tooltip
                  contentStyle={{ background: "rgba(255,255,255,0.95)", borderRadius: 8, border: "1px solid #dfeae5", fontSize: 12 }}
                  formatter={(value: number, _name: string) => [`${value} 条`, "帖子数"]}
                />
          ...
          {stats.channelComments.length > 0 ? (
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={stats.channelComments} margin={{ top: 4, right: 16, left: -8, bottom: 0 }}>
                ...
                <Tooltip
                  contentStyle={{ background: "rgba(255,255,255,0.95)", borderRadius: 8, border: "1px solid #dfeae5", fontSize: 12 }}
                  formatter={(value: number, _name: string) => [`${value} 条`, "评论数"]}
                />
          ...
          {stats.channelUsers.length > 0 ? (
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={stats.channelUsers} margin={{ top: 4, right: 16, left: -8, bottom: 0 }}>
                ...
                <Tooltip
                  contentStyle={{ background: "rgba(255,255,255,0.95)", borderRadius: 8, border: "1px solid #dfeae5", fontSize: 12 }}
                  formatter={(value: number, _name: string) => [`${value} 人`, "用户数"]}
                />
...
function countUniqueUsers(posts: StoredPost[], comments: StoredComment[], users: StoredUser[]) {
...
function profileToStoredUser(taskId: string, userId: string, profile: UserProfileData): StoredUser {
